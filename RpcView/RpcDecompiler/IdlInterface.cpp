...
{	
	char chUuidStr[100] = {0};
	char chUuidPathStr[120] = {0};
	...
	strcpy(chUuidPathStr,chUuidStr);
	strcat(chUuidStr,".py");
	
	WIN32_FIND_DATA pNextInfo;  
	HANDLE hFile = FindFirstFile(chUuidStr,&pNextInfo);
	
	if(INVALID_HANDLE_VALUE == hFile)
	{
		fstream fp(chUuidStr,ios::app);
		fp << "from rpc_forge import *" << std::endl;
		fp << "interface = Interface(\"" << chUuidPathStr << "\",(" << m_RpcIfId.VersMajor << "," << m_RpcIfId.VersMinor << "), [" << std::endl;
		fp << m_ossComplexTypes.str();
		for(size_t i=0; i< m_szNbFunctions; i++)
		{
			fp << std::endl;
			if(m_vectFunctions[i] == NULL)
			{
				fp << "void Opnum"<<i<<"NotDecoded(void)";
			}
			else
			{
				fp << *(m_vectFunctions[i]);
			}
			fp << std::endl;
		}
		fp << "} ";		
