...
void InterfacesWidget_C::InterfaceSelected(const QModelIndex& Index)
{
	...
	QString	pUuidString         = pProxyModel->data( pProxyModel->index(Index.row(), Column_Uuid) ).toString().toLatin1().data();
	...
	if (UuidStringList.contains(pUuidString))
	{
		return;
	}
	UuidStringList.append(pUuidString);
...
End:
	...
	{
		QModelIndex parent; 
		QModelIndex index1 = pModel->index(Index, Column_Uuid, parent);  
		InterfaceSelected(index1);
		Index--;
	}
